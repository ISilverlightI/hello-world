...
/**
 * Ein Virtueller Spieler, welcher eine Zuordnung und Auswahl der zu spielenden
 * Werte, abhängig von den Summen und Werten um welche Rundenbezogen gespielt
 * wird, festlegt und an die Methode naechsterZug() aus der Klasse HolsDerGeier
 * zurück gibt. Spezialisierung der abstrakten Klasse HolsDerGeierSpieler
 * 
 * @author Jan Rohrbach
 */
// ??? @return zuSpielendeKarte: Gibt den Wert der Karte zurück, welche ich als
// nächstes spielen möchte.
public class Strategy1 extends HolsDerGeierSpieler {
	...
	/**
	 * Konstruktor dieser Klasse. Ungenutzt und bislang nicht benötigt.
	 */
	public Strategy1() {
	}
	...
	public void reset() {
		...
		// leert und füllt ArrayList nochZuGewinnenSortiert in bestimmter Reihenfolge
		// von 15 bis -1
	...
	/**
	 * Legt fest, welche Spielsituation vorliegt und verweist auf die passende
	 * Methode. Unterscheidet zwischen einem Stich oder einer standart Situation
	 * (kein Stich).
	 * 
	...
	public int gibKarte(int naechsteKarte) {
		...
		// Initialisierung der Variablen, für die zu spielende Karte, und der
		// Aktualisierung der zuletzt gespielten Karte des Gegners in der jeweiligen
		// Runde.
		...
		// aktualisierung der ArrayLists nochZuGewinnenSortiert, nochZuGewinnen,
		// nochNichtGespielt und der Variablen meineLetzteKarte zum Ende der Runde.
	...
	/**
	 * Legt fest auf Grund aller vorhandenen Daten, ob um die zu gewinnende Summe
	 * gespielt werden soll und verweist auf die zugehörige Methode. Unterscheidet
	 * zwischen mustHave (will ich DEFINITIV haben), noNeed (benötige ich NICHT),
	 * StandardPosSumme (Summe ist kleiner als die aktuelle höchste Karte, welche
	 * man noch bekommen kann, aber größer als 0.), StandartNegSumme (Summe ist
	 * kleiner als 0 aber größer als die aktuelle kleinste Karte, welche man noch
	 * bekommen kann).
	 * 
	 * @param naechsteKarte Wert der Karte, welche diese Runde aufgedeckt wurde.
	 * @param summe         Wert aller Karten, um welchen diese Runde gespielt wird.
	 * @return ret: Gibt den Wert der Karte, welcher ausgespielt werden soll, an die
	 *         Methode gibKarte weiter.
	 */
	private int stich(int naechsteKarte) {
		// Initialisierung der Variablen, für die zu spielende Karte, und der
		// Aktualisierung der Summe in der jeweiligen Runde.
		int ret = -99;
		summe = (summe + naechsteKarte);

		// Summe > höchste noch verfügbare Karte *WILL ICH HABEN* verweise auf
		// mustHave()
		...
		// Summe < niedrigste noch verfügbare Karte *WILL ICH NICHT HABEN* verweise auf
		// noNeed()
		else if (summe <= nochZuGewinnen.get(nochZuGewinnen.size() - 1)) {
	...
	/**
	 * Legt fest, welcher Wert standardmäßig für den Wert der vorliegenden Karte
	 * gespielt werden soll. Unterscheidet zwischen einem aktuellen positiven oder
	 * negativen Wert.
	 * 
	 * @param naechsteKarte Wert der Karte, welche diese Runde aufgedeckt wurde.
	 * @return ret: Gibt den Wert der Karte, welcher ausgespielt werden soll, an die
		...
		for (int i = 0; i < (nochZuGewinnenSortiert.size()); i++) {
			...
			if (naechsteKarte < 0) {
				...
				if (Math.abs(naechsteKarte) == Math.abs(nochZuGewinnenSortiert.get(i))) {
					...
					if (nochNichtGespielt.size() < 10 && vomGegnerNochNichtGelegt
							.contains(vomGegnerNochNichtGelegt.indexOf(nochNichtGespielt.get(i))) == true)
					{
	...
	/**
	 * Legt fest, welcher Wert in dieser speziellen Situation eines Stiches gelegt
	 * werden soll. Situation: Summe ist kleiner als 0 aber größer als die aktuelle
	 * kleinste Karte, welche man noch bekommen kann. Vergleicht die Summe der
	 * Karten, um die gespielt wird, mit allen Karten, welche man noch gewinnen kann
	 * und legt abhängig von der derzeitig offen liegenden Karte einen zu spielenden
	 * Wert fest.
	 * 
	 * @param naechsteKarte Wert der Karte, welche diese Runde aufgedeckt wurde.
	 * @param summe         Wert aller Karten, um welchen diese Runde gespielt wird.
	 * @return ret: Gibt den Wert der Karte, welcher ausgespielt werden soll, an die
	 *         Methode stich weiter.
	 */
	private int standardNegSumme(int naechsteKarte, int summe) {
		...
		// gibt Wert für zu spielende Karte an stich() oder selten auch an noNeed()
		// zurück
		return ret;
	}
	...
	/**
	 * Legt fest, welcher Wert in dieser speziellen Situation eines Stiches gelegt
	 * werden soll. Situation: Summe ist kleiner als die aktuelle höchste Karte,
	 * welche man noch bekommen kann, aber größer als 0. Vergleicht die Summe der
	 * Karten, um die gespielt wird, mit allen Karten, welche man noch gewinnen kann
	 * und legt abhängig von der derzeitig offen liegenden Karte einen zu spielenden
	 * Wert fest.
	 * 
	...
	/**
	 * Legt fest, welcher Wert in dieser speziellen Situation eines Stiches gelegt
	 * werden soll. Situation: aktuelle Summe wird NICHT benötigt. Legt abhängig von
	 * der derzeit offenliegenden Karte einen zu spielenden Wert fest und
	 * entscheidet im Falle einer negativen offenliegenden Karte, die Priorität der
	 * Summe und den davon abhängige Wert, welcher gespielt werden soll oder
	 * verweist gegebenenfalls auf eine passende Methode.
	 * 
	 * @param naechsteKarte Wert der Karte, welche diese Runde aufgedeckt wurde.
	 * @param summe         Wert aller Karten, um welchen diese Runde gespielt wird.
	 * @return ret: Gibt den Wert der Karte, welcher ausgespielt werden soll, an die
	...
	private int noNeed(int naechsteKarte, int summe) {
		...
		else if (nochZuGewinnen.get(0) < (-summe)) {
	...
	/**
	 * Legt fest, welcher Wert in dieser speziellen Situation eines Stiches gelegt
	 * werden soll. Situation: aktuelle Summe wird DEFINITIV benötigt. Gibt abhängig
	 * von der derzeit offen liegenden Karte an, ob der höchste oder niedrigste Wert
	 * gespielt werden soll, welchen ich auf meiner Hand besitze um diese Summe, um
	 * welche diese Runde gespielt wird, zu bekommen.
	 * 
	 * @param naechsteKarte Wert der Karte, welche diese Runde aufgedeckt wurde.
	 * @param summe         Wert aller Karten, um welchen diese Runde gespielt wird.
	 * @return ret: Gibt den Wert der Karte, welcher ausgespielt werden soll, an die
